...
- **生日年龄与生肖** — 「生日」分类自动按年重复，卡片与详情页显示「距离 20 岁 · 属狗」（生日当天显示实岁），生肖按春节切换
...
- 新增 🦌🦌记录器桌面小部件两款：2×1 状态条（已戒天数 / 今日状态 / 最长纪录）与 2×2 本月记录一览（每格带小字日期，破戒日为蓝格加 🦌、今天描边，含本月破戒次数），点击直达记录器
- 新增 「生日」内置分类：选择后按年自动重复，首页卡片与详情页显示「距离 20 岁 · 属狗」（生日当天转实岁），生肖按春节切换（用内置农历表换算）
